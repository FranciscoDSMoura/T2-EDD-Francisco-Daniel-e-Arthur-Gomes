...
int consultar_partidas(BDTime *bd_t, BDPartidas *bd_p, const char* caminho_arquivo){
    ...
    switch(opcao){
        case '1': buscar_por_mandante(bd_t, bd_p, 10);return 1; 
        case '2': buscar_por_visitante(bd_t, bd_p, 10); return 1; 
        case '3': buscar_por_m_ou_v(bd_t, bd_p, 10); return 1; 
        case '4': return 0;
...
void imprimir_tabela_classificacao(BDTime *bd_t, BDPartidas *bd_p){
    ...
    NoTime *atual = recebe_BDTime_inicio(bd);
    while (atual) {
        Time* t = recebe_time_do_no(atual);
        imprimir_time(t);
...
int imprimir_menu(BDTime *bd_t, BDPartidas *bd_p, const char* caminho_arquivo){
    char opcao;
    ...
    while (opcao != 'Q' && opcao != 'q')  {
        ...
        switch(opcao){
            ...
            case '2': consultar_partidas(bd_t, bd_p, caminho_arquivo); break;
            case '3': 
                if(consultar_partidas(bd_t, bd_p, caminho_arquivo) != 0)
                    atualizar_partida(bd_t, bd_p, caminho_arquivo); 
                break;
            case '4': 
                if(consultar_partidas(bd_t, bd_p, caminho_arquivo) != 0)
